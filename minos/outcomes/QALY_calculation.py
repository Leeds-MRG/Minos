...
def aggregate_csv(filename, intervention, year, start_year, subset_func_string=None):
    ...
    if "who_boosted" in subset_func_string:
        keep_columns.append("income_boosted")
    if "GBIS_boosted" in subset_func_string:
        keep_columns.append("GBIS_income_boosted")
    ...
    if len(df['alive'].value_counts())==1:
        dead_pop = 0
    else:
    ...
    if intervention == 'baseline':
        ...
        if 'income_boosted' not in df.columns:
