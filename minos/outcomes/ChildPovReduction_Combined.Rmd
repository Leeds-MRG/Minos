...
SF12_change_percentage <- function(summary_group, scen.list) {
  ...
  p1 <- ggplot(plot_data, aes(x = year, y = percent_change_SF_12)) +
    ...
    geom_smooth() +
    labs(title = tit, subtitle = 'GAM Smoothed') +
    xlab('Year') +
  ...
  p2 <- ggplot(plot_data_confint, aes(x = year, y = mean_SF12_change)) +
    ...
    labs(title = tit, subtitle = '95% Confidence Intervals') +
    ylab('Percentage Change in SF-12')
...
```{r}
SF12_change_percentage_num_priority_groups(scen.list.rel)
```
...
```{r}
SF12_change_percentage_num_priority_groups(scen.list.abs)
```
