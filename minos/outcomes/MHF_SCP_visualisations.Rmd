...
compare_gender <- function(comparison_scen) {
  ...
  ggsave(filename = paste0('gender_comparison_', comparison_scen, '.png'),
         plot <- p1,
         path = 'plots')
}

compare_subpop <- function(summary_group, comparison_scen) {
  
  if(summary_group == 'UC_gender') { comparison.group <- 'sex' }
  if(summary_group == 'priority_child_under_one_UC') { comparison.group <- 'child_under_one' }
  if(summary_group == 'priority_disabled_UC') { comparison.group <- 'disabled_family' }
  if(summary_group == 'priority_ethnicity_UC') { comparison.group <- 'ethnic_minority' }
  if(summary_group == 'priority_lone_parent_UC') { comparison.group <- 'lone_parent_flag' }
  if(summary_group == 'priority_mother_under_25_UC') { comparison.group <- 'mother_under_25' }
  if(summary_group == 'priority_mother_three_plus_children') { comparison.group <- 'three_plus_children' }
  
  scen.list <- c('baseline', comparison_scen)
  data <- do.call(rbind, generate_df_list(summary_group, scen.list))
  
  if(summary_group == 'UC_gender') { 
    data <- data %>%
      filter(universal_credit == 1)
  }
  
  data[[comparison.group]] <- factor(data[[comparison.group]])
  
  subpop.change <- data %>%
    select(run_id, .data[[comparison.group]], SF_12, mean_cost, year, scen) %>%
    pivot_wider(names_from = scen,
                values_from = c(SF_12, mean_cost),
                names_sep = '.') %>%
    mutate(across(starts_with("SF_12"), ~ . - SF_12.baseline)) %>%
    ...
    pivot_longer(cols = starts_with("diff") | starts_with("mean_cost"),
                 ...
                 names_sep = "\\.") %>%
    filter(scen != 'baseline') %>%
    select(run_id, .data[[comparison.group]], year, diff_SF_12)
  
  subpop.confint <- subpop.change %>%
    group_by(.data[[comparison.group]], year) %>%
    summarise(n = n(),
              mean_SF12_change = mean(diff_SF_12),
              margin = qt(0.975, df = n - 1) * (sd(diff_SF_12) / sqrt(n)),
              lower = mean_SF12_change - margin,
  ...
  return(subpop.confint)
  
  p1 <- ggplot(data = subpop.confint, aes(x = year, y = mean_SF12_change, group = .data[[comparison.group]], colour = .data[[comparison.group]], fill = .data[[comparison.group]])) +
    geom_ribbon(aes(ymin = lower, ymax = upper), linetype = 'dotted', alpha = 0.3) +
    geom_line(aes(color = .data[[comparison.group]]), linetype = 'dashed') +
    geom_point() +
    geom_hline(yintercept = 0, linetype = 'dashed') +
    scale_fill_manual(values = c("#E69F00", "#56B4E9")) +
    scale_color_manual(values = c("#E69F00", "#56B4E9")) +
    labs(subtitle = comparison_scen) +
    xlab('Year') +
    ylab('SF-12 MCS Change') +
    theme_minimal()
  
  p2 <- ggplot(data = subpop.confint, aes(x = year, y = mean_SF12_change, group = .data[[comparison.group]], colour = .data[[comparison.group]], fill = .data[[comparison.group]])) +
    geom_smooth() +
    geom_hline(yintercept = 0, linetype = 'dashed') +
    labs(subtitle = comparison_scen) +
    xlab('Year') +
  ...
  ggsave(filename = paste0(comparison.group, '_comparison_', comparison_scen, '.png'),
         plot = p1,
         path = 'plots')
}
```
...
compare_lone_parent <- function(comparison_scen) {
  ...
  lone_parent.confint <- lone_parent.change %>%
    ...
    summarise(n = n(),
              ...
              lower = mean_SF12_change - margin,
              upper = mean_SF12_change + margin) %>%
    rename(`Lone Parent` = lone_parent_flag)
  
  comparison_scen <- gsub("[a-zA-Z ]", "", comparison_scen)
  comparison_scen <- paste0("£", comparison_scen)
  
  lone_parent.confint$`Lone Parent` <- ifelse(lone_parent.confint$`Lone Parent` == 1, 'TRUE', 'FALSE')
  
  p1 <- ggplot(data = lone_parent.confint, aes(x = year, y = mean_SF12_change, group = `Lone Parent`, colour = `Lone Parent`, fill = `Lone Parent`)) +
    geom_ribbon(aes(ymin = lower, ymax = upper), linetype = 'dotted', alpha = 0.3) +
    geom_line(aes(color = `Lone Parent`), linetype = 'dashed') +
    geom_point() +
    geom_hline(yintercept = 0, linetype = 'dashed') +
  ...
  ggsave(filename = paste0('lone_parent_comparison_', comparison_scen, '.png'),
         plot <- p1,
         path = 'plots')
}
```
...
```{r mental_illness_risk_plots_subpop}
mental_illness_risk_plots_subpop <- function(summary_group, comparison_scen) {
  
  if(summary_group == 'UC_MIR_families_sex') { comparison.group <- 'sex' }
  if(summary_group == 'UC_MIR_families_lone_parent') { comparison.group <- 'lone_parent' }
  if(summary_group == 'UC_MIR_families_three_plus_children') { comparison.group <- 'three_plus_children' }
  if(summary_group == 'UC_MIR_families_ethnicity') { comparison.group <- 'ethnic_minority' }
  if(summary_group == 'UC_MIR_families_disabled_family') { comparison.group <- 'disabled_family' }
  if(summary_group == 'UC_MIR_families_mother_under_25') { comparison.group <- 'mother_under_25' }
  ...
  plot_data1 <- data %>%
    filter(mental_health_risk == 'TRUE') %>%
    #select(-prop, -mental_health_risk) %>%
    select(run_id, .data[[comparison.group]], count, year, scen) %>%
    pivot_wider(names_from = scen,
                values_from = count) %>%
    mutate(across(starts_with("£"), ~ . - baseline, .names = "diff_{.col}")) %>%
    select(run_id, year, .data[[comparison.group]], starts_with('diff')) %>%
    pivot_longer(cols = starts_with("diff"),
                 names_sep = '_',
                 names_to = c("diff", "scen"),
                 values_to = 'Count') %>%
    select(run_id, year, .data[[comparison.group]], Count)
  
  gender.confint <- plot_data1 %>%
    group_by(.data[[comparison.group]], year) %>%
    summarise(n = n(),
              Mean_Count = mean(Count),
              margin = qt(0.975, df = n - 1) * (sd(Count) / sqrt(n)),
  ...
  p1 <- ggplot(gender.confint, aes(x = year, y = Mean_Count, group = .data[[comparison.group]], colour = .data[[comparison.group]], fill = .data[[comparison.group]])) +
    geom_ribbon(aes(ymin = lower, ymax = upper), linetype = 'dotted', alpha = 0.3) +
    geom_line(aes(color = .data[[comparison.group]]), linetype = 'dashed') +
    geom_point() +
    geom_hline(yintercept = 0, linetype = 'dashed') +
  ...
  plot_data_2 <- data %>%
    filter(mental_health_risk == 'TRUE') %>%
    select(run_id, .data[[comparison.group]], prop, year, scen) %>%
    pivot_wider(names_from = scen,
                values_from = prop) %>%
    mutate(across(starts_with("£"), ~ . - baseline, .names = "diff_{.col}")) %>%
    select(run_id, year, .data[[comparison.group]], starts_with('diff')) %>%
  ...
  plot_data2_confint <- plot_data_2 %>%
    mutate(Proportion = Proportion * 100) %>%
    group_by(year, .data[[comparison.group]]) %>%
    summarise(n = n(),
              Mean_Proportion = mean(Proportion),
              margin = qt(0.975, df = n - 1) * (sd(Proportion) / sqrt(n)),
              lower = Mean_Proportion - margin,
  ...
  p2 <- ggplot(plot_data2_confint, aes(x = year, y = Mean_Proportion, group = .data[[comparison.group]], colour = .data[[comparison.group]], fill = .data[[comparison.group]])) +
    geom_ribbon(aes(ymin = lower, ymax = upper), linetype = 'dotted', alpha = 0.3) +
    geom_line(aes(color = .data[[comparison.group]]), linetype = 'dashed') +
    geom_point() +
    geom_hline(yintercept = 0, linetype = 'dashed') +
...
# 
# 
# # Q1: What is the total eligible sample in E&W?
# #   : UC_families_count * 100/scaling_factor
# 
# UC_families <- generate_df_list('UC_kids', 'baseline')
# UC_families <- do.call(rbind, UC_families)
# 
# UC_fam <- UC_families %>%
#   filter(universal_credit == 1,
#          children == TRUE,
...
# 
# # eligible sample
# eligible_sample_count <- mean_count * (100 / scaling_factor)
...
```{r}
compare_gender(comparison_scen = '10UniversalCredit')
...
# compare_subpop(summary_group = 'UC_gender', comparison_scen = '10UniversalCredit')
# compare_subpop(summary_group = 'UC_gender', comparison_scen = '25UniversalCredit')
# compare_subpop(summary_group = 'UC_gender', comparison_scen = '50UniversalCredit')
# compare_subpop(summary_group = 'UC_gender', comparison_scen = '70UniversalCredit')
# compare_subpop(summary_group = 'UC_gender', comparison_scen = '100UniversalCredit')
```
...
# compare_subpop(summary_group = 'priority_lone_parent_UC', comparison_scen = '10UniversalCredit')
# compare_subpop(summary_group = 'priority_lone_parent_UC', comparison_scen = '25UniversalCredit')
# compare_subpop(summary_group = 'priority_lone_parent_UC', comparison_scen = '50UniversalCredit')
# compare_subpop(summary_group = 'priority_lone_parent_UC', comparison_scen = '70UniversalCredit')
# compare_subpop(summary_group = 'priority_lone_parent_UC', comparison_scen = '100UniversalCredit')
```

## Child Under One

```{r}
compare_subpop(summary_group = 'priority_child_under_one_UC', comparison_scen = '10UniversalCredit')
compare_subpop(summary_group = 'priority_child_under_one_UC', comparison_scen = '25UniversalCredit')
compare_subpop(summary_group = 'priority_child_under_one_UC', comparison_scen = '50UniversalCredit')
compare_subpop(summary_group = 'priority_child_under_one_UC', comparison_scen = '70UniversalCredit')
compare_subpop(summary_group = 'priority_child_under_one_UC', comparison_scen = '100UniversalCredit')
```

## Child Under One

```{r}
compare_subpop(summary_group = 'priority_ethnicity_UC', comparison_scen = '10UniversalCredit')
compare_subpop(summary_group = 'priority_ethnicity_UC', comparison_scen = '25UniversalCredit')
compare_subpop(summary_group = 'priority_ethnicity_UC', comparison_scen = '50UniversalCredit')
compare_subpop(summary_group = 'priority_ethnicity_UC', comparison_scen = '70UniversalCredit')
compare_subpop(summary_group = 'priority_ethnicity_UC', comparison_scen = '100UniversalCredit')
```
...
#               upper = Mean_Count + margin)
#   
#   final_count <- plot_data1_confint %>%
...
# Mental Illness Risk SUBPOPULATIONS

## Gender

```{r}
sum <- 'UC_MIR_families_sex'

mental_illness_risk_plots_subpop(sum, comparison_scen = '10UniversalCredit')
mental_illness_risk_plots_subpop(sum, comparison_scen = '25UniversalCredit')
mental_illness_risk_plots_subpop(sum, comparison_scen = '50UniversalCredit')
mental_illness_risk_plots_subpop(sum, comparison_scen = '70UniversalCredit')
mental_illness_risk_plots_subpop(sum, comparison_scen = '100UniversalCredit')
```

## Lone Parents

```{r}
sum <- 'UC_MIR_families_lone_parent'
...
```{r}
sum <- 'UC_MIR_families_three_plus_children'
...
```{r}
sum <- 'UC_MIR_families_mother_under_25'
...
```{r}
sum <- 'UC_MIR_families_disabled_family'
