...
out.path <- here::here('output', 'default_config_batch2')
...
int.list <- list(base, int)
scen.list <- c('base', 'ChildPovReduction')
...
SF12.change.by.deciles <- function(df.list, scen.list, start.year = 2020, batch = FALSE) {
  ...
  int_cols <- grep("ChildPovReduction.SF12", names(combined), value = TRUE)
...
```{r}
SF12.by.simd.deciles(int.list, scen.list, start.year = 2020, batch = batch)
```
...
```{r}
SF12.change.by.deciles(int.list, scen.list, start.year = 2020, batch = batch)
```
