...
density_plot_quintile <- function(base, int, quintile) {
  ...
  p <- ggplot(combined, aes(x = hh_income, y = time, group = interaction(time, Source), color = Source)) +
    geom_density_ridges() +
    labs(title = 'Household Income Density Plots',
...
density_plot_quintile_2 <- function(base, int, quintile) {
  ...
  p <- ggplot(filtered_combined, aes(x = hh_income, y = time, group = interaction(time, Source), color = Source)) +
    geom_density_ridges() +
    labs(title = 'Household Income Density Plots',
         subtitle = paste0('Quintile ', quintile))
  return(p)
