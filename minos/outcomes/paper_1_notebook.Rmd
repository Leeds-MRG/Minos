...
if (load_cache) {
  ...
  #mice_set <- mice(data = mice.data[, mice_columns], #method=method,
  #                 m = n_iter, maxit = max_iter,
  #                 remove.collinear=T)
  
  mice_set <- futuremice(data = mice.data[, imp_columns], #method=method,
                   m = n_iter, minit = max_iter,
                   remove.collinear=T, progressor = progressor)
...
iextrapolated.weights.data <- do.call(rbind, lapply(extrapolated_weights.filelist, read.csv))
...
if (reflect) {
    ...
    glmm.mice.long.subset[, c(depend)] <- max_value - glmm.mice.long.subset[, c(depend)] 
  }
if (yeo_johnson)
{
...
glmm.fitted <- (max_value - predict(yj, predict(mice.glmm.object, type='response') + min_value +rnorm(predict(mice.glmm.object, type='response'),0,0.2), inverse=T))#
#glmm.fitted <- (max_value - predict(mice.glmm.object, type='response') + min_value + rnorm(nrow(glmm.mice.long.subset), 0, 1))
glmm.fitted <- trunc(glmm.fitted, 0 , 100)
qq_plot(residuals(mice.glmm.object, type='pearson'), paste0(save.path, "glmm_qq.pdf"))
residual_density_plot(res=residuals(mice.glmm.object) , file_name=paste0(save.path, "glmm_residual_density.pdf"), guide="normal")

squareRootRes <- sqrt(residuals(mice.glmm.object, type='pearson'))
...
for (i in 1:7){
  rolling.data$SF_12 <- max_value - predict(yj,  predict(mice.glmm.object, start.data, type='response',allow.new.levels=T) + rnorm(nrow(rolling.data),0,0.25)+min_value, inverse=T)
