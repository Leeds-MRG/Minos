...
out.path <- here::here('output', 'SIPHER7asd/')
...
all.combined <- rbind(base.subset, energy.subset)
all.combined$scenario <- factor(all.combined$scenario, levels = c('Baseline', 'EnergyDownlift'))
...
income_equivinc_change_percent <- function(base.dat, int.dat) {
  base.inc <- base.dat %>%
    select(pidp, time, hh_income, equivalent_income, weight) %>%
    group_by(time) %>%
    summarise(base_income = weighted.mean(hh_income, w = weight, na.rm = TRUE),
  ...
  ggplot(combined, aes(x = time, y = percent_change, group = variable, color = variable)) +
    geom_line() +
    geom_hline(yintercept = 0, linetype = 'dashed')
}

income_equivinc_change(base.dat, energy.dat)
income_equivinc_change_percent(base.dat, energy.dat)
