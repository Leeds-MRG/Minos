...
int.dat <- read_singular_local_out(out.path, 'ChildPovertyReduction', drop.dead = TRUE)
...
The first intervention we will focus on is a reduction in the prevalence of children in relative poverty. 
...
ggplot(hh_boosted, aes(x = time, y = n_boosted)) +
  geom_line() +
...
ggplot(hh_boosted, aes(x = time, y = kids_boosted)) +
  geom_line() +
  geom_vline(xintercept = 2030, linetype = 'dashed') +
...
ggplot(hh_boosted, aes(x = time, y = total_boost)) +
  geom_line() +
  geom_vline(xintercept = 2030, linetype = 'dashed') +
...
ggplot(mean.boost, aes(x = time, y = mean_boost)) +
  geom_line() +
  geom_vline(xintercept = 2030, linetype = 'dashed') +
...
```{r}
boosted.overtime <- int.dat %>%
  filter(income_boosted == 'True')

base.all.SF12 <- base.dat %>%
  select(time, weight, SF_12, nkids, pidp) %>%
  filter(pidp %in% boosted.overtime$pidp) %>%
  group_by(time) %>%
  summarise(base.SF12 = weighted.mean(SF_12, w = weight))
