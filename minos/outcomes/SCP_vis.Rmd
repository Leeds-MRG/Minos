...
#out.path <- here::here('output', 'scaled_scotland_small')
#out.path <- here::here('output', 'tmp_default_baseline')


# define subset of variables to reduce size of batch output
var.list <- c('age', 'ethnicity', 'region', 'nkids', 'nkids_ind', 'S7_labour_state',
...
save.path <- here::here('plots', 'SCOTGOVWORK', 'CPR')
...
SF12.change.treated.batch <- function(df.list, scen.list) {
  ...
  df.list <- mapply(batch.mean, df.list, scen.list, boosted, SIMPLIFY = FALSE)
...
hh_boosted <- int25.test %>%
  ...
  mutate(prop_boosted = n_boosted / n)
	#prop_kids_boosted = # MORE HERE

p1 <- ggplot(hh_boosted, aes(x = time, y = n_boosted)) +
  geom_line() +
  labs(title = 'Number of households receiving intervention', subtitle = 'int25') +
  xlab('Year') +
  ylab('Number Boosted')
print(p1)
...
p2 <- ggplot(hh_boosted, aes(x = time, y = kids_boosted)) +
  geom_line() +
  labs(title = 'Number of children in households receiving intervention', subtitle = 'int25') +
  xlab('Year') +
