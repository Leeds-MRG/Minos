...
def subset_minos_data(data, subset_func_string, mode='default_config'):
...
def aggregate_lineplot(df, destination, prefix, v, method):
    ...
    if v in variable_name_map.keys():
        y_label = variable_name_map[v]
    else:
        y_label = v

    if method == weighted_nanmean:
        y_label += " Weighted Mean Relative Difference"
    else:
        y_label += f" {method.__name__}"
