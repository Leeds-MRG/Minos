...
#workingDir <- "/home/luke/Documents/WORK/MINOS/CPR_ARC_SUMMARIES/old/old6/"
...
scen.list.test <- c('baseline', 'ChildPovertyReductionSUSTAIN')
...
SF12_change_percentage(summary_group = 'treated_relative',
                       scen.list = scen.list.test)
```
...
plot_data <- data %>%
  group_by(scen, year, mental_health_risk) %>%
  summarise(count = mean(count)) %>%
  filter(mental_health_risk == 'TRUE')

ggplot(plot_data, aes(x = year, y = count, group = scen, color = scen)) +
  geom_point() +
  geom_line()
...
plot_data_2 <- data %>%
  ...
  pivot_wider(names_from = scen,
              values_from = count) %>%
  mutate(MHR_diff = ChildPovertyReductionRELATIVE_2 - baseline) %>%
  select(run_id, year, MHR_diff) %>%
  group_by(year) %>%
  summarise(MHR_diff = mean(MHR_diff))

ggplot(data = plot_data_2, aes(x = year, y = MHR_diff)) +
  geom_point() +
  geom_line()
