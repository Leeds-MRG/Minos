...
summary_group <- 'whole_pop_income_quint'
scen.list.test <- c('baseline', 'ChildPovertyReductionRELATIVE_2')
...
base.test <- filter(data, scen == 'baseline')
pov.test <- filter(data, scen == 'ChildPovertyReductionRELATIVE_2')

# ggplot(base.test, aes(x = year, y = SF_12, group = income_quintile, color = income_quintile)) +
#   geom_smooth(alpha = 0.7) +
#   geom_smooth(data = pov.test, aes(x = year, y = SF_12, group = income_quintile, color = income_quintile), linetype = 'dotted')
...
ggplot(data_combined, aes(x = year, y = SF_12, group = interaction(income_quintile, scenario), color = income_quintile, linetype = scenario)) +
  geom_smooth(alpha = 0.7) +
  scale_color_manual(name = "Income Quintile", 
                     values = c("red", "blue", "green", "orange", "purple")) +
  scale_linetype_manual(name = "Scenario", 
                        values = c("Baseline" = "solid", "Intervention" = "dotted")) +
  labs(title = "SF_12 Scores by Income Quintile",
       subtitle = "Comparison of Baseline and Intervention",
       x = "Year",
       y = "Mean SF_12 MCS") +
  theme_minimal() +
  theme(legend.position = "right",
...
ggplot(data_combined, aes(x = year, y = SF_12, group = scenario, color = scenario)) +
  geom_smooth(alpha = 0.7, aes(linetype = scenario)) +
  facet_wrap(~ income_quintile) +
  scale_color_manual(name = "Scenario", 
                     values = c("Baseline" = "black", "Intervention" = "red")) +
  scale_linetype_manual(name = "Scenario", 
                        values = c("Baseline" = "solid", "Intervention" = "dotted")) +
  labs(title = "SF_12 Scores by Income Quintile",
       subtitle = "Comparison of Baseline and Intervention Scenarios",
       x = "Year",
       y = "SF_12 Score") +
  theme_minimal() +
...
diff_data <- data %>%
  pivot_wider(names_from = scen, values_from = SF_12) %>%
  mutate(difference = `ChildPovertyReductionRELATIVE_2` - baseline) %>%
  gather(scen, SF_12, baseline, ChildPovertyReductionRELATIVE_2)

ggplot(diff_data, aes(x = year, y = difference, group = income_quintile, color = income_quintile)) +
  geom_line() +
  scale_color_manual(name = "Income Quintile", 
                     values = c("red", "blue", "green", "orange", "purple")) +
  labs(title = "Difference in SF_12 Scores by Income Quintile",
       subtitle = "Intervention Scenario Minus Baseline Scenario",
       x = "Year",
       y = "Difference in SF_12 Score") +
  theme_minimal() +
  theme(legend.position = "right",
...
# Aggregate data
agg_data <- data %>%
  group_by(year, income_quintile, scen) %>%
  summarize(mean_SF_12 = mean(SF_12, na.rm = TRUE)) %>%
  spread(scen, mean_SF_12) %>%
  mutate(difference = `ChildPovertyReductionRELATIVE_2` - baseline)

ggplot(agg_data, aes(x = year, y = difference, fill = income_quintile)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(name = "Income Quintile", 
                    values = c("red", "blue", "green", "orange", "purple")) +
  labs(title = "Average Difference in SF_12 Scores by Income Quintile",
       subtitle = "Intervention Minus Baseline",
       x = "Year",
       y = "Difference in SF_12 Score") +
  theme_minimal() +
  theme(legend.position = "right",
