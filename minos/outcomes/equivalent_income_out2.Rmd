---
title: "Equivalent Income Outcome Visualisation - 2"
output:
  html_document:
    toc: true
    toc_float: true
...
```{r}
#S7.var.list <- c('hh_income', 'equivalent_income', 'housing_quality', 'neighbourhood_safety', 'S7_physical_health', 'S7_mental_health', 'S7_labour_state', 'loneliness')
...
```{r}
S7_ordinal_pathway_vis(base.dat, energy.dat, 'EnergyCrisis', 'housing_quality')
# S7_ordinal_pathway_vis_batch(base.batch, energy.batch, 'EnergyCrisis', 'housing_quality')
```
...
```{r}
S7_ordinal_pathway_vis(base.dat, energy.dat, 'EnergyCrisis', 'neighbourhood_safety')
# S7_ordinal_pathway_vis_batch(base.batch, energy.batch, 'EnergyCrisis', 'neighbourhood_safety')
```
...
combined <- merge(base.hous, int.hous, by = c('time', 'housing_quality'))

combined$diff <- combined$EnergyCrisis - combined$Baseline



ggplot(combined, aes(x = time, y = diff, group = housing_quality, color = housing_quality)) +
  geom_line() +
  geom_point()
```
...
base.dat.2 <- base.dat
base.dat.2$housing_quality2[base.dat.2$housing_quality == 'No to all'] <- 1
base.dat.2$housing_quality2[base.dat.2$housing_quality == 'Yes to some'] <- 2
base.dat.2$housing_quality2[base.dat.2$housing_quality == 'Yes to all'] <- 3
wage.dat.2 <- energy.dat
wage.dat.2$housing_quality2[wage.dat.2$housing_quality == 'No to all'] <- 1
wage.dat.2$housing_quality2[wage.dat.2$housing_quality == 'Yes to some'] <- 2
wage.dat.2$housing_quality2[wage.dat.2$housing_quality == 'Yes to all'] <- 3

base.hous.mean <- base.dat.2 %>%
  select(pidp, time, housing_quality2, weight) %>%
  group_by(time) %>%
  summarise(mean_housing = weighted.mean(housing_quality2, w = weight, na.rm = TRUE)) %>%
  mutate(source = 'Baseline')
...
# df <- combine_and_pivot_long(base.dat, 'Baseline', wage.dat, 'LivingWage', c('housing_quality', 'income_boosted'))
# 
