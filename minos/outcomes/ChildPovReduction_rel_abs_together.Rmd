...
#workingDir <- "/home/luke/Documents/WORK/MINOS/Minos/output/batch2_cpr_default/cpr_sum_out/"
#workingDir <- "/home/luke/Documents/MINOS/CPR_ARC_SUMMARIES/old/old_33_scot_clipped/"
#workingDir <- "/home/luke/Documents/WORK/MINOS/CPR_ARC_SUMMARIES/old/old_46_reverted_try3_40/"
...
```{r "priority_subgroups", include=FALSE}
priority_subgroup_plot <- function(priority_group, scen) {
  
  priority_group <- paste0('priority_', priority_group)
  scen <- paste0(scen, '_together')
  
  #print(priority_group)
  #print(scen)
  
  data <- do.call(rbind, generate_df_list(priority_group, scen))
  
  #return(data)
  
  plot_data <- data %>%
    select(run_id, SF_12, mean_cost, year, scenario) %>%
    pivot_wider(names_from = scenario,
                values_from = c(SF_12, mean_cost),
                names_sep = '.') %>%
    mutate(across(starts_with("SF_12"), ~ . - SF_12.baseline)) %>%
  ...
  # ggsave(filename = paste0(priority_group, '_smooth', '.png'),
  #        plot = p1)
  # 
  # ggsave(filename = paste0(priority_group, '_confint', '.png'),
  #        plot = p2)
}

# test <- priority_subgroup_plot(priority_group = 'child_under_one',
#                                scen = 'ChildPovertyReduction')
```


```{r}
# priority_subgroup_plot(priority_group = 'child_under_one',
#                                scen = 'ChildPovertyReduction')
# priority_subgroup_plot(priority_group = 'ethnicity',
#                                scen = 'ChildPovertyReduction')
# priority_subgroup_plot(priority_group = 'mother_under_25',
#                                scen = 'ChildPovertyReduction')
# priority_subgroup_plot(priority_group = 'three_plus_children',
#                                scen = 'ChildPovertyReduction')
```
...
Results disaggregated by if household belongs to 0, 1, 2+ priority subgroups.
