...
base <- read_batch_out_all_years_summarise(out.path, 
                                            scenario = 'baseline',
                                            verbose = TRUE)
int25 <- read_batch_out_all_years_summarise(out.path, 
                      scenario = '25UniversalCredit',
                      verbose = TRUE)
# int30 <- read_batch_out_all_years_summarise(out.path,
#                       scenario = '30UniversalCredit',
#                       verbose = TRUE)
# int35 <- read_batch_out_all_years_summarise(out.path,
#                       scenario = '35UniversalCredit',
#                       verbose = TRUE)
# int40 <- read_batch_out_all_years_summarise(out.path,
#                       scenario = '40UniversalCredit',
#                       verbose = TRUE)
# int45 <- read_batch_out_all_years_summarise(out.path,
#                       scenario = '45UniversalCredit',
#                       verbose = TRUE)
int50 <- read_batch_out_all_years_summarise(out.path,
                      scenario = '50UniversalCredit',
                      verbose = TRUE)
...
int.list <- list(base, int25, int50)
scen.list <- c('base', 'int25', 'int50')

# make sure all data are in dataframes
int.list <- lapply(int.list, as.data.frame)
...
test <- SF12.change.treated.batch(int.list, scen.list)
```
...
# # 
# # test2 <- batch.mean(base, 'base')
# 
# 
# test3 <- base %>%
#   select(time, run_id, SF_12) %>%
#   group_by(time, run_id) %>%
#   summarise(base.SF12= mean(SF_12, na.rm =TRUE))
```

```{r}
# test4 <- as.data.frame(int25) %>%
#   dplyr::select(time, run_id, SF_12)
```
...
```{r}
SF12.by.simd.deciles(int.list, scen.list, batch = batch)
```

```{r}
SF12.by.simd.deciles(int.list, scen.list, start.year = 2024, batch = batch)
```
...
```{r}
SF12.change.by.deciles(int.list, scen.list, batch = batch)
```
