...
NOTE this is for generating a synthetic population (seeded using CRN) only. Use the TestPopulation
...
class BasePopulation:
    ...
    # define config defaults (if not specified in config yaml file)
    # minimum and maximum ages of simulants.
    # this is mildly naive but noone has hit 125 yet.
    ...
    # special property for vivarium builder
    @property
    def sub_components(self):
        return self._sub_components

    def setup(self, builder):
        # load in populaton config and input data directory from yaml.
        self.config = builder.configuration.population
        input_config = builder.configuration.input_data
        # loading in CRN for general purpose and aging
        ...
        # method to register population to builder
        self.register_simulants = builder.randomness.register_simulants
        # columns to add to population view
        columns = ['age', 'sex', 'alive', 'location', 'entrance_time', 'exit_time']
        # add input simulants to population view using generate_base_population
        self.population_view = builder.population.get_view(columns)
        builder.population.initializes_simulants(self.generate_base_population,
                                                 creates_columns=columns)
        # loads 'population_structure' data from builder and creates an age column
        # at the midpoint of the age group.
        source_population_structure = load_population_structure(builder)
        # add location from builder
