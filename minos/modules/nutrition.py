...
class geeYJNutrition(Base):
    ...
    def setup(self, builder):
        ...
        # just load this once.
        self.gee_transition_model = r_utils.load_transitions(f"nutrition_quality/gee_yj/nutrition_quality_GEE_YJ", self.rpy2Modules,
    ...
    def on_time_step(self, event):
        ...
        newWaveNutrition.index = pop.index
        #newWaveNutrition['nutrition_quality'] = newWaveNutrition['nutrition_quality'].astype(float)
        newWaveNutrition['nutrition_quality'] = np.clip(newWaveNutrition['nutrition_quality'], 0, 30)
        # Draw individuals next states randomly from this distribution.
        # Update population with new income
        self.population_view.update(newWaveNutrition['nutrition_quality'])
    ...
    def calculate_nutrition(self, pop):
        ...
        nextWaveNutrition = r_utils.predict_next_timestep_yj_gaussian_gee(self.gee_transition_model,
                                                                       self.rpy2Modules,
                                                                       self.history_data,
        ...
        return nextWaveNutrition.iloc[self.history_data.loc[self.history_data['time']==self.year].index]
    # Special methods used by vivarium.
    @property
    def name(self):
        return 'geeYJNutrition'
