...
class Tobacco(Base):
    ...
    def calculate_tobacco(self, pop):
        ...
        if not self.transition_model or year <= 2020:
            self.transition_model = r_utils.load_transitions(f"ncigs/zip/ncigs_{year}_{year + 1}",
                                                             self.rpy2Modules,
                                                             path=self.transition_dir)
            if not self.cross_validation:  # randomise fixed effects causes zip models to fail in cross-validation. Can't figure out why so switching it off for now
