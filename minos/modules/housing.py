...
class Heating(Base):
    ...
    def on_time_step(self, event):
        ...
        heating_prob_df = self.calculate_housing(pop)
        ...
        heating_prob_df["heating"] = self.random.choice(heating_prob_df.index,
                                                        ...
                                                        heating_prob_df)
        #heating_prob_df.index = heating_prob_df.index.astype(float)
    ...
    def calculate_housing(self, pop):
        ...
        # load transition model based on year.
        year = min(self.year, 2018)
        transition_model = r_utils.load_transitions(f"heating/logit/heating_{year}_{year+1}")
        # returns probability matrix (3xn) of next ordinal state.
        prob_df = r_utils.predict_next_timestep_logit(transition_model, pop, 'heating')
        prob_df.columns = [1.]
        return prob_df