...
class glmmMWB(Base):
    ...
    def calculate_mwb(self, pop):
        ...
        """
        year = min(self.year, 2018)
        transition_model = r_utils.load_transitions(f"SF_12/glmm/sf12_glmm")
        #np.max(pop['SF_12'])
        return self.max_sf12 - r_utils.predict_next_timestep_glmm(transition_model, pop, 'SF_12')
...
class geeMWB(Base):
    ...
    def calculate_mwb(self, pop):
        ...
        """
        year = min(self.year, 2018)
        transition_model = r_utils.load_transitions(f"SF_12/gee/SF_12_GEE")
        return self.max_sf12 - r_utils.predict_next_timestep_gee(transition_model, pop, 'SF_12')
