...
class ChildPovertyReductionSUSTAIN(Base):
    ...
    def on_time_step(self, event):
        ...
        logging.info("INTERVENTION:")
        logging.info(
            f"\tApplying effects of the child relative poverty reduction intervention in year {event.time.year}...")
        ...
        target_pop['boost_amount'][(target_pop['income_boosted'] is True) &  # previously uplifted
                ...
                relative_poverty_threshold - target_pop[
            'hh_income'] + 1)  # boost is difference between income and threshold (+1 to guarantee above threshold)
        ...
        print(
            f"Proportion of children in poverty AFTER intervention: {still_in_pov.groupby('hidp').first()['nkids'].sum() / nkids_total}")
