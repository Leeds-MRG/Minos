...
SF_12_MCS.stats <- diff_by_whole_period_quintile(raw.dat, 'SF_12_MCS')
```
...
Quick look at SF_12_MCS_MCS also.

```{r}
spaghetti_plot(old.dat, 'SF_12_MCS')
spaghetti_plot(new.dat, 'SF_12_MCS')
spaghetti_plot(raw.dat, 'SF_12_MCS')
```
...
```{r}
violin_plot(old.dat, 'SF_12_MCS')
violin_plot(new.dat, 'SF_12_MCS')
violin_plot(raw.dat, 'SF_12_MCS')
```
...
sf12.mod <- readRDS(paste0(mod.path, 'SF_12_MCS/ols2/SF_12_MCS_2017_2018.rds'))
...
#raw <- raw.cv %>%
#  filter(!hh_income %in% miss.values, !SF_12_MCS %in% miss.values) %>%
#  group_by(pidp) %>%
#  mutate(diff.hh_income = hh_income - lag(hh_income, order_by = time),
#         diff.SF12_MCS = SF_12_MCS - lag(SF_12_MCS, order_by = time)) %>%
#  filter(time != 2009)
...
#            mean.SF12 = mean(SF_12_MCS, na.rm = TRUE))
