...
- [x] Check what age people achieve each level of education
  - [x] Average age plot (see if change over time in raw)
  - [x] Histograms
  - [ ] Animated histograms?
...
- [x] Make sure stock population is doing max_educ prediction
...
raw.files <- list.files(here::here('data', 'stock_US'), pattern='[0-9]{4}_US_cohort.csv', full.names = TRUE)
...
ggplot(data = raw.meanAge, aes(x = education_state, y = age)) +
  geom_col() +
  geom_text(aes(label = format(round(age, 2), nsmall = 2)), vjust = -0.5)
...
## Summary stats by educ_state

```{r}
raw.by.educ <- raw.dat %>%
  group_by(education_state) %>%
  summarise(hh_income = mean(hh_income, na.rm = TRUE),
            SF12 = mean(SF_12, na.rm = TRUE))

raw.by.educ$education_state <- as.factor(raw.by.educ$education_state)

ggplot(raw.by.educ, aes(x = education_state, y = hh_income)) +
  geom_col()

ggplot(raw.by.educ, aes(x = education_state, y = SF12)) +
  geom_col()
```

## Transition into FT Education

How many people transition into full time education? How many do this after the age of 30?

```{r}
raw.eductrans <- raw.dat %>%
  select(pidp, time, age, education_state, max_educ, S7_labour_state) %>%
  mutate(backInEduc = (S7_labour_state == 'FT Education') & (lag(S7_labour_state, order_by = time) != 'FT Education')) %>%
  filter(backInEduc == TRUE)

ggplot(raw.eductrans, aes(x = age)) +
  geom_histogram() +
  labs(title = 'Age came back to FT Education')
```


## Max Education Predictions

```{r}
raw.maxEduc <- raw.dat %>%
  dplyr::filter(age < 31) %>%
  dplyr::select(pidp, time, age, education_state, max_educ, S7_labour_state)

raw.maxEducDiff <- raw.maxEduc %>%
  dplyr::filter(max_educ != education_state)
```
...
Test that retirement is working as planned. Make sure that all 65+ are in the not working category in outputs.

```{r}
retired <- base.dat %>%
  select(pidp, time, age, S7_labour_state) %>%
  filter(age >= 65) %>%
  group_by(S7_labour_state) %>%
  count()

ggplot(retired, aes(x = S7_labour_state, y = n)) +
  geom_col()

retired.by.ageGroup <- base.dat %>%
  select(pidp, time, age, S7_labour_state) %>%
  filter(age >= 65) %>%
  mutate(age.group = cut(age, breaks = 0,65,70,75,80,85,90,120)) %>%
  group_by(age.group, S7_labour_state) %>%
  count()
...
spaghetti_plot(raw.s, 'education_state',
               save = FALSE)
