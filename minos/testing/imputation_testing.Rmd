...
compare_ordinal <- function(final, complete, imputed, var) {
  ...
  p2 <- ggplot(data = merged, mapping = aes(x = time, y = n, group = .data[[var]], colour = .data[[var]])) +
    geom_line() +
    geom_point() +
    facet_grid(.~source) +
  ...
  p3 <- ggplot(data = var.norm, mapping = aes(x = time, y = prct, fill=.data[[var]], position='dodge')) +
