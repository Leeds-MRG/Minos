...
combined.QALY.change <- combined %>%
  ...
  pivot_wider(names_from = 'intervention',
              values_from = 'QALYs') %>%
  mutate(QALYs_Support = energyDownlift - baseline,
         QALYs_NoSupport = energyDownliftNoSupport - baseline) %>%
  select(run_id, year, QALYs_Support, QALYs_NoSupport) %>%
  pivot_longer(cols = QALYs_Support:QALYs_NoSupport,
               names_prefix = 'QALYs_',
...
ggplot(combined.QALY.change, aes(x = QALYs, group = intervention, colour = intervention, fill = intervention)) +
  geom_histogram() +
  facet_wrap(~year) +
  labs(title = 'QALY change by run', subtitle = 'All Years') +
  xlab('QALY Change')
...
combined.cost <- combined %>%
  ...
  select(run_id, year, pop_size, total_boost, intervention, QALYs) %>%
