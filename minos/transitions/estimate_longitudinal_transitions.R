...
run_longitudinal_models <- function(transitionDir_path, transitionSourceDir_path, mod_def_name, data, mode)
{
  ...
  valid_longitudnial_model_types <- c("LMM", "LMM_DIFF", "GLMM", "GEE_DIFF","ORDGEE", "CLMM", "NNET2")
  ...
  repeat{
    ...
    else if (dependent %in% c("SF_12", 'marital_status', 'education_state'))  {
    ...
    } else if (tolower(mod.type) == 'nnet2') {
      model <- estimate_yearly_nnet(data = sorted_df,
                                    formula = form,
