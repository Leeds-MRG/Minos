...
energy_data <- readxl::read_xlsx(here::here("..", "data", "energy_prices_simpler_yearly.xlsx"))
# primary energy material sources. fuel and motor oil omitted due to no UKHLS data. 
...
m1 <- arima(electric.series, order=c(7, 0, 1))
print(m1)
...
pdf(here::here("plots", "electric_forecast.pdf"))
autoplot(forecast(m1, 15))
dev.off()
...
pdf(here::here("plots", "gas_forecast.pdf"))
autoplot(forecast(m2, 15))
dev.off()
# residuals appear to be in nominal range. large LB p-value also support no autocorrelation between residuals. 
```
...
pacf(solid.series)
#arima.electric 
m3 <- arima(solid.series, order=c(5, 1 ,5))
#Box.test(residuals(m1), type="Ljung-Box")
plot(residuals(m3))
...
pdf(here::here("plots", "solid_forecast.pdf"))
autoplot(forecast(m3, 15))
dev.off()
```
...
# ACF, difference 1 ACF and PACF suggests AR(2)  difference 1 and MA(4) structure (5, 0, 1) in ARIMA.
acf(liquid.series)
...
pdf(here::here("plots", "oil_forecast.pdf"))
autoplot(forecast(m4, 15))
dev.off()
# residuals appear to be in nominal range. large LB p-value also support no autocorrelation between residuals. 
```
...
create.if.not.exists(here::here("data", "transitions", "energy_prices"))
write.csv(electric.series, here::here("data", "transitions", "energy_prices", "solid_historic.csv"))
write.csv(gas.series, here::here("data", "transitions", "energy_prices", "gas_historic.csv"))
write.csv(solid.series, here::here("data", "transitions", "energy_prices", "solid_historic.csv"))
write.csv(liquid.series, here::here("data", "transitions", "energy_prices", "liquid_historic.csv"))
write.csv(forecast(m1, 15), here::here("data", "transitions", "energy_prices", "electric_arima.csv"))
write.csv(forecast(m2, 15), here::here("data", "transitions", "energy_prices", "gas_arima.csv"))
write.csv(forecast(m3, 15), here::here("data", "transitions", "energy_prices", "solid_arima.csv"))
write.csv(forecast(m4, 15), here::here("data", "transitions", "energy_prices", "liquid_arima.csv"))
```