...
time_horizon <- 40 # how long to forecast energy data for
...
m1 <- arima(electric.series, order=c(5, 0, 1), method='ML')
print(m1)
...
pacf(gas.series)
#arima.electric 
m2 <- arima(gas.series, order=c(4, 0 ,1), method='ML')
#Box.test(residuals(m1), type="Ljung-Box")
plot(residuals(m2))
autoplot(forecast(m2, time_horizon))
checkresiduals(m2)
print(m2$aic)
# residuals appear to be in nominal range. large LB p-value also support no autocorrelation between residuals. 
...
pacf(solid.series)
#arima.electric 
m3 <- arima(solid.series, order=c(5, 0 ,1), method='ML')
#Box.test(residuals(m1), type="Ljung-Box")
plot(residuals(m3))
autoplot(forecast(m3, time_horizon))
checkresiduals(m3)
print(m3$aic)
# residuals appear to be in nominal range. large LB p-value also support no autocorrelation between residuals. 
...
pacf(liquid.series)
#arima.electric 
m4 <- arima(gas.series, order=c(5, 0 ,1), method='ML')
#Box.test(residuals(m1), type="Ljung-Box")
plot(residuals(m4))
autoplot(forecast(m4, time_horizon))
checkresiduals(m4)
print(m4$aic)
# residuals appear to be in nominal range. large LB p-value also support no autocorrelation between residuals. 
...
write.csv(forecast(m1, time_horizon), "persistent_data/energy_prices/electric_arima.csv")
write.csv(forecast(m2, time_horizon), "persistent_data/energy_prices/gas_arima.csv")
write.csv(forecast(m3, time_horizon), "persistent_data/energy_prices/solid_arima.csv")
write.csv(forecast(m4, time_horizon), "persistent_data/energy_prices/liquid_arima.csv")