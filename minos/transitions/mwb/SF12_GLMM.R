...
format_sf12_glmm_transition_data <- function(source, years, v){
...
data <- format_sf12_glmm_transition_data("data/final_US/", c(2016,2017,2018,2019), "SF_12")
formula <- "I(max(SF_12) - SF_12 + 0.001)  ~
  sex + 
  ethnicity + 
  age + 
  ...
  (1|pidp)"
dep.glmm.gamma <- glmer(formula,  nAGQ=0, family=Gamma(link='log'), data = data)

sf12.glmm.filename <- "data/transitions/SF_12/glmm/sf12_glmm.rds"
saveRDS(dep.glmm.gamma, file=sf12.glmm.filename)
...
glmm.gamma.preds <- predict(dep.glmm.gamma, newdata=recent_data, allow.new.levels=T,type='response')
glmm.gamma.preds <- max(data$SF_12) - glmm.gamma.preds


#hist(glmm.gamma.preds)
#res <- residuals(dep.glmm.gamma)
