...
def calculate_children(data,
    ...
    default_value = 0
    data.loc[data['nnewborn'].isin(US_utils.missing_types), 'nnewborn'] = default_value
    ...
    for i, pidp in enumerate(pidps):
        ...
        if (subframe['nnewborn'].sum() > default_value) and not (subframe['nkids_ind_raw'].lt(default_value).any()):
