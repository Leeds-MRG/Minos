...
def main(input_raw_data, year):
    ...
    # child_name = US_utils.US_file_name(year, "../UKDA-6614-stata/stata/stata13_se/", "child")
    ...
    #
    # # Remove orphans, i.e. any households with no adults
    ...
    # # print(sum(np.abs(actual_children - declared_children)))  # Sum absolute error
    #
    # # Get children and join ages into string separated by underscores
    ...
    # # Maybe these are all newborns? There are three in one year, may be triplets but unlikely
    ...
    #
    # # Households with nkids > 0 but no children in the child dataset must be assigned error values
    # who_children_not_in_data = (collapsed_children_US['child_ages'].isna()) & (collapsed_children_US['nkids']>0.0)
    # collapsed_children_US.loc[who_children_not_in_data, 'nkids'] *= 0  # Force children to 0 in these households with children not in the dataset (newborns?)
