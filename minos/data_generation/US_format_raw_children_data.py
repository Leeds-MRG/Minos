...
def main(input_raw_data, year):
    ...
    child_data.columns = ['hidp', 'age', 'pidp']
    ...
    input_raw_data['is_child'] = np.where(input_raw_data['age'] < 16, True, False)
    input_raw_data['is_adult'] = np.where(input_raw_data['age'] > 15, True, False)
    ...
    collapsed_children_US_with_children = input_raw_data.merge(right=child_data, on=['pidp'], how='left', suffixes=('', '_delme'))
