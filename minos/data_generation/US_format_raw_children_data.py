...
def main(input_raw_data, year):
    ...
    chained_ages = pd.DataFrame(final_US_with_children.loc[final_US_with_children['is_child'] == True, ].groupby('hidp', as_index=False)['age'].apply('_'.join))
    chained_ages.columns = ['child_ages', 'hidp']
