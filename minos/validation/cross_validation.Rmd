...
cv <- read_singular_local_out(out.path, 'baseline')
...
savepath <- '/home/luke/Documents/WORK/MINOS/VALIDATION_PLOTS/Cross-Validation/'
```
...
```{r}
sf12.m.pivoted <- combine_and_pivot_long(df1 = cv, 
                                       df1.name = 'simulated', 
                                       df2 = raw, 
                                       df2.name = 'raw', 
                                       var = 'SF_12_MCS')

plot_mean_comparison(pivoted.df = sf12.m.pivoted, 
                     var = 'SF_12_MCS', 
                     group.var = 'scenario',
                     save = TRUE,
                     save.path = savepath)

compare_boxplot_long(pivoted.df = sf12.m.pivoted,
                     var = 'SF_12_MCS',
                     scen1 = 'raw',
                     scen2 = 'simulation')
```
...
```{r}
sf12.m.t.tests <- two_sample_t_test(cv, 'cv', raw, 'raw', 'SF_12_MCS')
sf12.m.t.tests
```

```{r}
yearly_box_plots(pivoted.df = sf12.m.pivoted,
                 var = 'SF_12_MCS',
                 scen1 = 'raw',
                 scen2 = 'simulation')
```
...
```{r}
sf12.p.pivoted <- combine_and_pivot_long(df1 = cv, 
                                       df1.name = 'simulated', 
                                       df2 = raw, 
                                       df2.name = 'raw', 
                                       var = 'SF_12_PCS')

plot_mean_comparison(pivoted.df = sf12.p.pivoted, 
                     var = 'SF_12_PCS', 
                     group.var = 'scenario',
                     save = TRUE,
                     save.path = savepath)

compare_boxplot_long(pivoted.df = sf12.p.pivoted,
                     var = 'SF_12_PCS',
                     scen1 = 'raw',
                     scen2 = 'simulation')
```
...
```{r}
sf12.p.t.tests <- two_sample_t_test(cv, 'cv', raw, 'raw', 'SF_12_PCS')
sf12.p.t.tests
```

```{r}
yearly_box_plots(pivoted.df = sf12.p.pivoted,
                 var = 'SF_12_PCS',
                 scen1 = 'raw',
                 scen2 = 'simulation')
```
