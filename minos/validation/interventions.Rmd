...
sf12.energy <- energy %>% 
  select(pidp, time, SF_12_MCS, income_boosted)
# first rename and cbind
sf12.b <- rename(sf12.base, base = SF_12_MCS)
sf12.e <- rename(sf12.energy, energy = SF_12_MCS)
...
ggplot(data = sf12.merged.byyr, mapping = aes(x = time, y = SF_12_MCS, color=scenario)) +
  geom_line()
```
...
# first rename and cbind
sf12.b <- rename(sf12.base, base = SF_12_MCS)
sf12.e <- rename(sf12.poverty, poverty = SF_12_MCS)
sf12.merged <- merge(sf12.b, sf12.e, by=c('pidp', 'time'))
sf12.merged <- pivot_longer(data = sf12.merged,
                              cols = base:poverty,
                              names_to = 'scenario',
