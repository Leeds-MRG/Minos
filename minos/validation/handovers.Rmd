...
rm(raw.hhs_kids.ethnicity, base.hhs_kids.ethnicity)
```
...
rm(raw.hh_kids_prop, base.hh_kids_prop, combined)
```
...
```{r}
density_plot_quintile <- function(raw, base, quintile) {
  
  raw <- raw %>%
    filter(income_quint == quintile) %>%
    select(pidp, time, hh_income) %>%
    rename(Final_US = hh_income)
  base <- base %>%
    filter(income_quint == quintile) %>%
    select(pidp, time, hh_income) %>%
  ...
  combined <- combined %>%
    pivot_longer(cols = Final_US:Baseline,
                 names_to = 'Source',
                 values_to = 'hh_income')
...
```{r}
mean_income_quintile <- function(raw, base) {
  raw <- raw %>%
...
tit <- 'Low Income & Material Deprivation Child'
...
```{r}
raw.s <- raw.dat %>%
  select(pidp, age, time, low_income_matdep_child)
base.s <- base.dat %>%
  select(pidp, age, time, low_income_matdep_child)

spag <- rbind(raw.s, base.s)

spaghetti_plot(spag, 'low_income_matdep_child',
               save = shall.we.save,
               save.path = save.path)
...
```{r}
prepped.list <- validation_prep_ordinal(raw.dat, base.dat, 'matdep_child')
...
# Now plot
ggplot(data = var, mapping = aes(x = time, y = n, group = matdep_child, colour = matdep_child)) +
  geom_line() +
  geom_vline(xintercept=start.year, linetype='dotted') +
  labs(title = tit, subtitle = 'Counts by Level') +
  xlab('Year') +
...
ggplot(data = norm, mapping = aes(x = time, y = prct, fill=matdep_child)) +
  geom_bar(stat = 'identity') +
  geom_vline(xintercept=start.year, linetype='dotted') +
  labs(title = tit) +
  xlab('Year') +
...
if (shall.we.save) {
  ggsave(filename = 'matdep_child.png',
         plot = last_plot(),
         path = save.path)
}
...
```{r}
raw.s <- raw.dat %>%
  select(pidp, age, time, matdep_child)
base.s <- base.dat %>%
  select(pidp, age, time, matdep_child)

spag <- rbind(raw.s, base.s)

spaghetti_plot(spag, 'matdep_child',
               save = shall.we.save,
               save.path = save.path)
