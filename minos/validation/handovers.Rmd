...
raw.files <- list.files(here::here('data', 'stock'), pattern='[0-9]{4}_US_cohort.csv', full.names = TRUE)
...
raw.fsit <- raw.dat %>% 
  ...
  filter(time > 2009) %>%
  group_by(time) %>%
  summarise(financial_situation = mean(financial_situation, na.rm = TRUE)) %>%
  mutate(source = 'final_US')
...
# merge before plot
fsit <- rbind(raw.fsit, base.fsit)

# Now plot
ggplot(data = fsit, mapping = aes(x = time, y = financial_situation, group = source, colour = source)) +
  geom_line() +
  geom_vline(xintercept=start.year, linetype='dotted') +
  labs(title = 'Financial Situation', subtitle = 'Full Sample') +
  xlab('Year') +
  ylab('Average')
...
fsit2 <- rbind(raw.fsit2, base.fsit2)

# Now plot
ggplot(data = fsit2, mapping = aes(x = time, y = financial_situation, group = source, colour = source)) +
  geom_line() +
  geom_vline(xintercept=start.year, linetype='dotted') +
  labs(title = 'Financial Situation') +
  xlab('Year') +
...
if (shall.we.save) {
  ggsave(filename = 'financial_situation_wav1.png',
       plot = last_plot(),
       path = save.path)
}
...
rm(raw.fsit, base.fsit, fsit, raw.wave1, raw.fsit2, base.fsit2, fsit2)
#rm(raw.phealth.check, base.phealth.check)
```
...
```{r}
raw.s <- raw.dat %>%
  select(pidp, age, time, financial_situation)
base.s <- base.dat %>%
  select(pidp, age, time, financial_situation)

spag <- rbind(raw.s, base.s)

spaghetti_plot(spag, 'financial_situation',
             save = shall.we.save,
             save.path = save.path)
