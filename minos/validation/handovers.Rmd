...
```{r}
prepped.list <- validation_prep_ordinal(raw.dat, base.dat, 'marital_status')
...
# Now plot
ggplot(data = var, mapping = aes(x = time, y = n, group = marital_status, colour = marital_status)) +
  geom_line() +
  geom_vline(xintercept=start.year, linetype='dotted') +
  labs(title = tit, subtitle = 'Counts by Level') +
  xlab('Year') +
...
ggplot(data = norm, mapping = aes(x = time, y = prct, fill=marital_status)) +
  geom_bar(stat = 'identity') +
  geom_vline(xintercept=start.year, linetype='dotted') +
  labs(title = tit) +
  xlab('Year') +
...
if (shall.we.save) {
  ggsave(filename = 'marital_status.png',
         plot = last_plot(),
         path = save.path)
}
...
```{r}
raw.s <- raw.dat %>%
  select(pidp, age, time, marital_status)
base.s <- base.dat %>%
  select(pidp, age, time, marital_status)

spag <- rbind(raw.s, base.s)

spaghetti_plot(spag, 'marital_status',
               save = shall.we.save,
               save.path = save.path)
