...
title: "`r paste(params$file_path, ' Handover Plots')`"
output:
...
if (file_path == 'default_MICE/') {
  raw.file_path <- 'imputed_final_US'
} else {
  raw.file_path <- 'final_US'
}

# Read raw datafiles in
raw.files <- list.files(here::here('data', raw.file_path), pattern='[0-9]{4}_US_cohort.csv', full.names = TRUE)
...
out.path <- here::here('output', file_path)
base.dat <- read_singular_local_out(out.path, 'baseline', drop.dead = TRUE)
```
...
# LA 16/2/24 - We still want the actual handover point though so we need both data points in the start year
# have changed to achieve this
