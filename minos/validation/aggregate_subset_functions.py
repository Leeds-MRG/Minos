...
def find_subset_function(function_string):
    ...
    subset_dict = {"none": None,
                        ...
                        "who_scottish_and_kids": who_scottish_and_kids,
                        "who_scottish_below_living_wage": who_scottish_below_living_wage,
                        "who_scottish_below_poverty_line_and_kids": who_scottish_below_poverty_line_and_kids,
...
def who_scottish_and_kids(df):
    """
    Who Scottish and alive and kids.
    """
    df = who_alive(df)
    df = who_scottish(df)
    return who_kids(df)
...
def who_scottish_below_poverty_line_and_kids(df):
    "who alive, scottish, and below poverty line and has kids?. Defined as 60% of national median hh income."
    df = who_scottish_and_kids(df)
    return df.loc[df['hh_income'] <= (np.nanmedian(df['hh_income']) * 0.6), ]
