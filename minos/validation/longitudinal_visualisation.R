...
trim <- function(x){
  x[(x > mean(x)-5*IQR(x)) & (x < mean(x)+5*IQR(x))]
}

density_plot <- function(pred_data, real_data, v) {
  hist(trim(pred_data[, c(v)]), breaks=100, freq=F)
  lines(density(real_data[, c(v)]), col='red')
}
...
main <- function() {
  ...
  real_data <- read.csv("data/final_US/2020_US_cohort.csv")
  ...
  print(spaghetti_plot(data, 'hh_income'))
  #sf12_violin2 <- violin_plot(data2, "SF_12")
  #sf12_spag2 <- spaghetti_plot(data2, "SF_12")
  density_plot(data[which(data$time==2035), ], real_data, "hh_income")
