...
def house_quality_intervention(
    ...
    conditions = [df_in['housing_quality'] == 'High',   # Best
                  df_in['housing_quality'] == 'Medium',
                  df_in['housing_quality'] == 'Low']    # Worst
    choices = [1, 2, 3]

    df_in['house_quality_score'] = np.select(conditions, choices)
    df_in['house_quality_score'] = np.where(
        (df_in['intervention'] == 1) & (df_in['house_quality_score'] > 1),
        df_in['house_quality_score'] - 1, df_in['house_quality_score']
    )

    conditions = [df_in['house_quality_score'] == 1,
                  df_in['house_quality_score'] == 2,
                  df_in['house_quality_score'] == 3]
    ...
    df_in.drop(['house_quality_score', 'intervention', 'msoa_code', ], axis=1, inplace=True)
    ...
    # 3. Run the pipeline
    args = Namespace(config=os.path.join(ROOT_DIR, 'config/glasgow_scaled.yaml'),
                     intervention=None,
                     runID=None,
                     runtime=None,
                     subdir='SF12_glasgow')
    ...
    indicator_sf12_mean = df['SF_12'].median()
    ...
    indicators_decile = {
        'sf12_mean_per_decile': df_decile_grouped['SF_12'].median(),
