...
print(paste0("Number of unique run_ids in base.batch: ", len(unique(base.batch$run_id))))
...
print(paste0("Number of unique run_ids in wage.batch: ", len(unique(wage.batch$run_id))))
...
print(paste0("Number of unique run_ids in energy.batch: ", len(unique(energy.batch$run_id))))
write.csv(energy.batch, file = 'output/energy_batch_aggregated.csv')
rm(energy.batch)
child.batch <- read_batch_out_all_years(out.path.batch, 'hhIncomeChildUplift', start.year = 2021, var.list = c('income_boosted', S7.var.list), verbose=TRUE)
print(paste0("Number of unique run_ids in child.batch: ", len(unique(child.batch$run_id))))
write.csv(child.batch, file = 'output/child_batch_aggregated.csv')
rm(child.batch)
pov.batch <- read_batch_out_all_years(out.path.batch, 'hhIncomePovertyLineChildUplift', start.year = 2021, var.list = c('income_boosted', S7.var.list), verbose=TRUE)
print(paste0("Number of unique run_ids in pov.batch: ", len(unique(pov.batch$run_id))))
