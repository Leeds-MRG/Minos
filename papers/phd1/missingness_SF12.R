...
main <- function(cached){
  ...
  data$region <- relevel(factor(data$region), ref="London") # level against london
  data$sex <- factor(data$sex)
  data$labour_state <- factor(data$labour_state)
  data$job_sec <- relevel(factor(data$job_sec), ref="-10") # level against -10 (unemployed)
  ...
  if (cached){
    ...
    pidps <- data$pidp
    save(pidps, file = 'papers/phd1/data/mice_pidps.RData')
    mice.weights <- data$weight
    ...
    save(mice_set, file = 'papers/phd1/data/mice_set.RData')
  ...
  save(mice.lm.object, file = 'papers/phd1/data/mice_lm_object.RData')
...
# much faster if just want plots.
main(F)
