...
main <- function(years){
  ...
  for (year in years){
    kept_columns <- c("pidp",
                      ...
                      "region",
                      "hh_income",
                      paste0(paste0("X", year), "_weight"),
                      paste0(paste0("X", year+5), "_weight"))
    
    data_files <- get.extrapolated_weight.files(source, year, year+1)
    new_data <- data_files$data1
    new_data <- new_data[, kept_columns]
  ...
  sf12.lm.weighted.2013 <- lm(formula, weights = data$year0_weight, data = data)
  print(summary(sf12.lm))
  forest_plot(sf12.lm.weighted.2013, 'papers/phd1/plots/extrapolated_SF12_forest_2013.pdf')
  
  sf12.lm.weighted.2018 <- lm(formula, weights = data$year5_weight, data = data)
  print(summary(sf12.lm))
  forest_plot(sf12.lm.weighted.2018, 'papers/phd1/plots/extrapolated_SF12_forest_2018.pdf')
  ...
  # Fit LASSO
  cv.lasso <- cv.glmnet(x, y, family = "gaussian", weights=data$year0_weight)
  lasso <- glmnet(x, y, alpha = 1,  family = "gaussian", weights = data$year0_weight)
  ...
  cv.lasso2 <- cv.glmnet(x, y, family = "gaussian", weights=data$year5_weight)
  lasso2 <- glmnet(x, y, alpha = 1, family = "gaussian", weights = data$year5_weight)
  
  print(coef(cv.lasso2, s = "lambda.min"))
  print(coef(cv.lasso2, s = "lambda.1se"))
